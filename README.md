...
- 四个按钮：
  ...
  - `清除`（tooltip：清除当前检测结果）
- 输出格式：`职业 玩家名 补正值`
  - 设置“计算补正总数”开启（默认）时，补正值 = **2x+y**；
  - 关闭时，补正值显示为 **x★+y**。
  - `x`：辅助职业大师（Status 4226）的层数
  - `y`：所穿新月岛装备“特殊修正”效果之和
  - 本人按装备栏直接计算 y；其余小队成员通过游戏“查看装备”流程读取装备后计算 y，读取失败时显示 `?`。
- 悬浮窗结果按“职业 / 名称 / 补正”三列对齐显示，按补正值从高到低排序，行间交替底色。
- 检测结果会同步显示在对应玩家的名牌上：在玩家名字（称号）后方显示补正值，字号比名字小 2 号（参照 Distance 插件的名牌文本实现，仅新月岛地域生效）。
- 单击结果表格的“名称”表头可对玩家名称模糊处理（以 █ 遮盖），再次单击取消模糊。
- 套装规则：
...
| 计算补正总数 | 开启 | 勾选显示 2x+y，未勾选显示 x★+y |
| 聊天输出 | 关闭 | 开启后通过 `/p` 将检测结果发送至小队频道（队内玩家可见） |
| 名牌显示补正 | 开启 | 在玩家名牌的名字（称号）后方显示补正值，字号比名字小 2 号（仅新月岛地域） |
